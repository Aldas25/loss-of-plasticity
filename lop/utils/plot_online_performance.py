...
def generate_online_performance_plot_for_subplot(
        ax,
        performances=None,
        colors=None,
        xticks=[],
    ...
    for index_hyper_param_label in range(shape[0]):
        ...
        if labels is not None:
            label = labels[index_hyper_param_label]
        color = colors[index_hyper_param_label]
        ax.plot(x*m, mean, '-', label=label, color=color)
        ax.fill_between(x*m, mean - std_err, mean + std_err, color=color, alpha=0.2)
    ...
    if caption is not None:
        ax.set_title(caption, fontsize=fontsize+2)
